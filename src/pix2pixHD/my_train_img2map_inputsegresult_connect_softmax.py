...
def eval_fidiou(args, model_G,model_seg, data_loader):
    ...
    for i, sample in enumerate(data_loader):
        ...
        seg_softmax = soft_outputs.detach().type(torch.FloatTensor).to(device)  # bs*1*h*w
        imgs_plus = torch.cat((imgs, seg_softmax), 1)  # bs*4*h*w
...
def train(args, get_dataloader_func=get_pix2pix_maps_dataloader):
    ...
    G = get_G(args,input_nc=3+args.label_nc) # 3+3，带上分割网络的预测softmax图，该图通道数=label数
    D = get_D(args)
    ...
    for epoch in range(epoch_now, args.epochs):
        ...
        for step, sample in enumerate(data_loader):
            ...
            imgs = sample['A'].to(device)
            maps = sample['B'].to(device)
            seg_softmax = soft_outputs.detach().type(torch.FloatTensor).to(device)  # bs*3*h*w
            imgs_plus = torch.cat((imgs, seg_softmax), 1)  # bs*6*h*w
