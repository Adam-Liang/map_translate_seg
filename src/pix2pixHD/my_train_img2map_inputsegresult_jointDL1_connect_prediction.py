...
from src.pix2pixHD.myutils import pred2gray_bytensor, gray2rgb,pred2gray
...
def train(args, get_dataloader_func=get_pix2pix_maps_dataloader):
    ...
    for epoch in range(epoch_now, args.epochs):
        ...
        for step, sample in enumerate(data_loader):
            ...
            imgs = sample['A'].to(device)
            maps = sample['B'].to(device)
            seg_ret = pred2gray_bytensor(outputs).unsqueeze(1).type(torch.FloatTensor).to(device) #bs*1*h*w
            # seg_ret2=pred2gray(outputs).unsqueeze(1).type(torch.FloatTensor).to(device) #bs*1*h*w
            # assert (seg_ret==seg_ret2).all()
            ...
            # grads_dlv3p_1 = 0
            # for name_1, param_1 in DLV3P.named_parameters():
            #     grads_dlv3p_1 += torch.abs(param_1.grad).sum().cpu().item()
            # seg_loss.backward()
            # grads_dlv3p_2 = 0
            # for name_1, param_1 in DLV3P.named_parameters():
            #     grads_dlv3p_2 += torch.abs(param_1.grad).sum().cpu().item()
            # print(grads_dlv3p_1)
            # print(grads_dlv3p_2)
            # print(grads_dlv3p_1==grads_dlv3p_2)
            G_optimizer.step()
            DLV3P_global_optimizer.step()  # (perform optimization step)
            DLV3P_backbone_optimizer.step()
