...
def train(args, get_dataloader_func=get_pix2pix_maps_dataloader):
    ...
    for epoch in range(epoch_now, args.epochs):
        ...
        for step, sample in enumerate(data_loader):
            ...
            seg_loss.backward(retain_graph=True)  # (compute gradients)
            grads_dlv3p_1=0
            ...
            if args.tensorboard_log and step % args.tensorboard_log == 0:  # defalut is 5
                ...
                sw.add_scalar('gardsum/fromGloss', grads_dlv3p_2, total_steps)
                sw.add_scalar('gardsum/fromsegloss', grads_dlv3p_1, total_steps)
