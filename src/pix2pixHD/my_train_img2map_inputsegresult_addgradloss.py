...
# 该文件新增实验内容
from src.pix2pixHD.try_change_model.grad_loss_L1 import L1_rgb_grad_loss
from src.pix2pixHD.try_change_model.grad_loss_struct import grad_loss
...
def train(args, get_dataloader_func=get_pix2pix_maps_dataloader):
    ...
    DLV3P_global_optimizer = torch.optim.Adam([{'params': seg_global_params, 'initial_lr': args.seg_lr_global}],
                                              lr=args.seg_lr_global, betas=(args.beta1, 0.999))
    DLV3P_backbone_optimizer = torch.optim.Adam([{'params': seg_backbone_params, 'initial_lr': args.seg_lr_backbone}],
                                                lr=args.seg_lr_backbone, betas=(args.beta1, 0.999))
    ...
    DLV3P_global_scheduler = torch.optim.lr_scheduler.LambdaLR(DLV3P_global_optimizer,
                                                               lr_lambda=lambda epoch: (1 - epoch / args.epochs) ** 0.9,
                                                               last_epoch=epoch_now)
    DLV3P_backbone_scheduler = torch.optim.lr_scheduler.LambdaLR(DLV3P_backbone_optimizer, lr_lambda=lambda epoch: (
                                                                                                                               1 - epoch / args.epochs) ** 0.9,
                                                                 last_epoch=epoch_now)
    ...
    GRADL1Loss = L1_rgb_grad_loss
    GRADstructLoss = grad_loss
    ...
    for epoch in range(epoch_now, args.epochs):
        ...
        for step, sample in enumerate(data_loader):
            ...
            grad_l1_loss=GRADL1Loss(fakes, maps)
            grad_struct_loss=GRADstructLoss(fakes,maps)+GRADstructLoss(fakes.transpose(-1,-2), maps.transpose(-1,-2))
            G_loss += (args._0701arg_gradloss_L1_alpha * grad_l1_loss +args._0701arg_gradloss_struct_alpha * grad_struct_loss)
            grad_l1_loss=grad_l1_loss.mean().item()
            grad_struct_loss=grad_struct_loss.mean().item()
            ...
            data_loader.write(f'Epochs:{epoch}  | Dloss:{D_loss:.6f} | Gloss:{G_loss:.6f}'
                              ...
                              f'| LLloss:{ll_loss:.6f} |grad_l1_loss:{grad_l1_loss:.6f}|grad_struct_loss:{grad_struct_loss:.6f}| lr_gan:{get_lr(G_optimizer):.8f}'
            ...
            if args.tensorboard_log and step % args.tensorboard_log == 0:  # defalut is 5
                ...
                sw.add_scalar('Loss/grad_l1', grad_l1_loss, total_steps)
                sw.add_scalar('Loss/grad_struct', grad_struct_loss, total_steps)
        ...
        if epoch % 10 == 0 or epoch == (args.epochs - 1):
            ...
            fid, iou = eval_fidiou(args, model_G=G, model_seg=DLV3P,
                                   data_loader=get_pix2pix_maps_dataloader(args2, train=False))
