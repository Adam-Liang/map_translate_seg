...
from src.pix2pixHD.deeplabv3plus.deeplabv3plus_moreparms_1.backbone import build_backbone
from src.pix2pixHD.deeplabv3plus.deeplabv3plus_moreparms_1.ASPP import ASPP


class deeplabv3plus(nn.Module):
    def __init__(self, cfg, parms_ratio=4.3602610696231662797432470395007):
        def more_parm(channel):
            return int(channel*(parms_ratio**0.5))
        cfg.MODEL_ASPP_OUTDIM = more_parm(cfg.MODEL_ASPP_OUTDIM)
        cfg.MODEL_SHORTCUT_DIM = more_parm(cfg.MODEL_SHORTCUT_DIM)

        super(deeplabv3plus, self).__init__()
        self.backbone = None
        self.backbone_layers = None
        input_channel = more_parm(2048)
        ...
        self.backbone = build_backbone(cfg.MODEL_BACKBONE,pretrained=False, os=cfg.MODEL_OUTPUT_STRIDE,parms_ratio=parms_ratio)
    ...
    def forward(self, x):
        ...
        feature_map=self.upsample4(feature_map)
