...
class AlignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        # 角度&宽度
        self.dir_thetas=os.path.join(opt.dataroot, opt.phase+"_theta")
        self.theta_paths = sorted(make_dataset(self.dir_thetas))
        assert len(self.theta_paths)==len(self.AB_paths)
        self.dir_wids = os.path.join(opt.dataroot, opt.phase + "_wid")
        self.wid_paths = sorted(make_dataset(self.dir_wids))
        assert len(self.wid_paths) == len(self.AB_paths)
    ...
    def __getitem__(self, index):
        ...
        # 角度宽度相关
        theta_path = self.theta_paths[index]
        wid_path=self.wid_paths[index]
        theta=np.load(theta_path)
        wid=np.load(wid_path)
        theta=theta[h_offset:h_offset + self.opt.fineSize,
              w_offset:w_offset + self.opt.fineSize]
        wid=wid[h_offset:h_offset + self.opt.fineSize,
              w_offset:w_offset + self.opt.fineSize]
        ...
        if (not self.opt.no_flip) and random.random() < 0.5:
            ...
            theta=np.flip(theta,axis=1) # 前面的是tensor，这俩是ndarray
            wid=np.flip(wid,axis=1)
        ...
        return {'A': A, 'B': B, 'seg':seg,'A_seg':A_seg,'B_seg':B_seg,
                'A_paths': AB_path, 'B_paths': AB_path, 'theta':theta , 'wid':wid}
