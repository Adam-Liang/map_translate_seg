...
if __name__=='__main__':
    floder=r'D:\map_translate\202009tw训练\tw数据-多层使用\未分训练测试'
    floder_new=r'D:\map_translate\202009tw训练\tw数据-多层使用'
    ...
    for f1 in list1:
        for f2 in list2:
            ...
            source_img_floder=os.path.join(floder,f1,f2)
            new_img_floder_train=os.path.join(floder_new,'train'+f1,f2)
            new_img_floder_test = os.path.join(floder_new, 'test'+f1, f2)
            ...
            for i in tqdm(range(total_num)):
                if i<A_num:
                    path_new=os.path.join(new_img_floder_train,get_inner_path(imgs_path[i],source_img_floder))
                else:
                    path_new=os.path.join(new_img_floder_test,get_inner_path(imgs_path[i],source_img_floder))
                make_floder_from_file_path(path_new)
                shutil.copy(imgs_path[i], path_new)
                num += 1

            print("data copy complete! %d img was processed, %d of train, %d of test" % (num,A_num,total_num-A_num))
