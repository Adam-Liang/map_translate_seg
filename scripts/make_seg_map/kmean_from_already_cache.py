...
def k_mean_fromfile(cache_path,k,Z0,N_file):
    G=[]
    Z=Z0 if len(Z0)==k else []
    # N=len(X)
    c=[]
...
def make_seg_img_from_maps(maps_path,segs_new_path,cache_path,one_time_num=500):
    if not os.path.isdir(cache_path):
        os.makedirs(cache_path)
    maps=make_dataset(maps_path)
    if not os.path.isdir(segs_new_path):
        os.makedirs(segs_new_path)
    if not os.path.isdir(os.path.join(segs_new_path, 'rgb')):
        os.makedirs(os.path.join(segs_new_path, 'rgb'))
    if not os.path.isdir(os.path.join(segs_new_path, 'seg')):
    ...
    # print('start k-means')
    # t1 = time.time()
    # Z,  k = k_mean_fromfile(cache_path, 5, stand_rgb,N_file=N_file)
    # t2 = time.time()
    # print('k-means 花费' + str(t2 - t1) + '秒')
    # with open(os.path.join(cache_path,'Z'), 'w') as f:
    ...
    for a in range(N_file):
        ...
        for i in range(one_time_num):
            ...
            new_im = gray2rgb(ret_new, label_list=[[239, 238, 236], [255, 93, 93], [170, 218, 255], [208, 236, 208],
                                                   [255, 242, 175]])
...
if __name__ == '__main__':
    ...
    # print(cnt)
    maps_path = r"D:\map_translate\看看效果\0601TW16_1708图_分割结果做p2p输入，epoch200，重新训练\fake_result"
    segs_new_path = r"D:\map_translate\看看效果\0601TW16_1708图_分割结果做p2p输入，epoch200，重新训练\fake_result_seg"
    cache_path = r"D:\map_translate\看看效果\0601TW16_1708图_分割结果做p2p输入，epoch200，重新训练\k_means_cache"
    make_seg_img_from_maps(maps_path,segs_new_path,cache_path)
