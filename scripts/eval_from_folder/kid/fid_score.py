...
from kid.models.lenet import LeNet5
...
def get_activations(files, model, batch_size=50, dims=2048,
    ...
    pred_arr = np.empty((n_used_imgs, dims)) # 数组元素为随机数据，不为空
...
def _compute_activations(path, model, batch_size, dims, cuda, model_type):
    if not type(path) == np.ndarray:
        ...
        tif = os.path.join(path, '*.tif')
        path = glob.glob(jpg) + glob.glob(png) + glob.glob(tif)# glob.glob 返回所有匹配文件列表
        if len(path) > 25000:
            import random
            random.shuffle(path)
            path = path[:25000]
...
def calculate_fid_given_paths(paths, batch_size, cuda, dims, bootstrap=True, n_bootstraps=10, model_type='inception'):
    ...
    for j, pth in enumerate(pths):
        ...
        with tqdm(range(n_bootstraps), desc='FID') as bar: # desc 进度条前方显示字符串
            for i in bar:
